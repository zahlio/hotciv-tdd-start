...
class UpdateTool extends NullTool {
	...
	public void mouseDown(MouseEvent e, int x, int y) {
		switch(count) {
		case 0: {
			editor.showStatus( "State change: Moving archer to (2,1)" );
		...
		case 5: {
			editor.showStatus( "Moving archer to (3,2) to slay blue legion " );
			game.moveUnit(new Position(2,2), new Position(3,2));
			break;
		}
		case 6: {
			editor.showStatus( "Moving archer to (4,1) to conquer blue city, this only works in real game " );
			game.moveUnit(new Position(3,2), new Position(4,1));
			break;
		}
		case 7:
			editor.showStatus("Settler performing it's action");
